...
def place(
    ...
    # 軟約束（grouping / boundary / mib-shape）的權重排程。
    # 2026-08-02: 翻轉方向 (DG-RePlAce 式：強->弱，而非原本弱->強) 全 100-case
    # 驗證後同時改善 Neutral 1.4606->1.4309 (-2.03%) 與 Real 1.1074->1.0682
    # (-3.54%)，Vgrp 173->168、Vmib 117->103、Vbnd 172->153 全降，且改善集中
    # 在高權重(高 n)案例——本 session 七次 Q/P 零和實驗以來首次雙贏。已升格
    # 為本檔預設；三個環境變數仍可覆寫回原本弱->強方向做對照
    # (ELECTRO_GRP0=0.2 ELECTRO_GRP1=1.8，BND/MIB 同理)。
    grp0 = float(os.environ.get("ELECTRO_GRP0", "1.8"))  # grouping penalty start
    grp1 = float(os.environ.get("ELECTRO_GRP1", "0.2"))  # grouping penalty end
    bnd0 = float(os.environ.get("ELECTRO_BND0", "1.8"))  # boundary penalty start
    bnd1 = float(os.environ.get("ELECTRO_BND1", "0.2"))  # boundary penalty end
    mib0 = float(os.environ.get("ELECTRO_MIB0", "1.8"))  # mib-shape penalty start
    mib1 = float(os.environ.get("ELECTRO_MIB1", "0.2"))  # mib-shape penalty end
    ...
    # ---- MIB 錨點的目標 log-aspect（ELECTRO_MIB_ANCHOR_SNAP=1）--------------
    # 這是從我們自己 `electro_optimized/` 路線移植回來的機制。那條線長期維持
    # **V_mib = 0**（2.4081 / 2.2113 / 2.1014 / 2.0361 / 2.1230 這幾個分數的
    # 報告全部都是 Vmib=0），而 slice_pack 這條線一直在 94-99 之間。
    #
    # 原理：MIB 群組若含 fixed/preplaced 錨點，錨點的 (w,h) 是釘死的，軟成員
    # 只要形狀跟它不同就一定計違規。把該群組的目標 log-aspect 設成
    # `log(tw/th)`，就能讓軟成員收斂到跟錨點相同的長寬比；又因為同群組共用
    # `area_sg`，形狀因此逐位元相同 → V_mib = 0。
    #
    # **為什麼要「訓練中引導 + 結束時對齊」兩段，而不是只在輸出階段硬覆寫**：
    # 2026-07-29 試過只在輸出階段直接覆寫 (w,h)（ELECTRO_MIB_ANCHOR），
    # 結果 **+0.8% 退步** —— 因為那是violent 的跳變，製造大量重疊丟給 legalize
    # 收拾，而且優化器全程都不知道這個目標。正確做法是全程用漸增權重的 L2 引導
    # （見下方 `mib_shape` loss），讓優化器自己收斂過去，結束時的對齊只是很小的
    # 修正。這個差異就是成功與失敗的分界。
    # 引導權重。舊路線在 Jacobi 初始化下掃到的最佳值是 0.03、隨機初始化是 0.05
    # （非單調的尖峰）；這裡沿用同樣的預設，可用環境變數覆寫再掃一次。
    _mib_default = "0.03" if os.environ.get("ELECTRO_JACOBI_MODE", "replace") == "replace" else "0.05"
    lam_mib_shape = float(os.environ.get("ELECTRO_MIB_SHAPE", _mib_default))
    la_target = torch.zeros(K, device=dev)
    has_anchor = torch.zeros(K, dtype=torch.bool, device=dev)
    if os.environ.get("ELECTRO_MIB_ANCHOR_SNAP", "0") == "1":
        for k in range(K):
            mems = (inv == k).nonzero().flatten()
            if mems.numel() < 2:
                continue                      # 單獨成組，沒有 MIB 約束
            anc = mems[is_fixed[mems] | is_pre[mems]]
            if anc.numel() == 0:
                continue                      # 純軟群組：共用 la 本來就同形
            i0 = int(anc[0].item())
            aw, ah = float(tw[i0]), float(th[i0])
            if aw <= 0 or ah <= 0:
                continue
            # 面積安全：錨點自己的面積必須就是群組共用面積，否則軟成員採用它的
            # 長寬比會讓 w*h 偏離自己的目標面積 → 踩到 1% 硬約束。
            if abs(aw * ah - float(area_sg[k])) > 1e-6 * max(1.0, float(area_sg[k])):
                continue
            la_target[k] = float(torch.log(torch.tensor(aw / ah)))
            has_anchor[k] = True
    ...
    for it in range(iters):
        ...
        lam_grp = grp0 + (grp1 - grp0) * frac
        lam_bnd = bnd0 + (bnd1 - bnd0) * frac

        # MIB 形狀引導：權重排程見上方 mib0/mib1（預設強->弱，2026-08-02 驗證）。
        # 讓優化器在早期佈局仍自由時就把有錨點群組的 log-aspect 拉向錨點值，
        # 收斂後期再放鬆，避免約束跟後段線長/密度微調搶同一份梯度預算。結束時的
        # `la.data.copy_` 因此只是很小的修正，不會撕裂已收斂的佈局。
        ...
        if has_anchor.any():
            loss = loss + (lam_mib_shape * (mib0 + (mib1 - mib0) * frac)) * mib_shape
        if edensity > 0.0:
    ...
    with torch.no_grad():
        # 結束時把有錨點群組的 log-aspect **精確**對齊錨點值。因為上面的漸增
        # 引導 loss 已經把 la 拉到接近 la_target，這一步只是最後的精確對齊
        # （消除殘留的浮點差），不會撕裂佈局。對齊後 `shapes()` 產生的軟成員
        # (w,h) 與錨點逐位元相同 → **V_mib = 0**。
        if has_anchor.any():
        ...
        # --- MIB 錨點形狀採用（ELECTRO_MIB_ANCHOR=1，opt-in）---------------
        # 混合群組（含 fixed/preplaced 錨點 + 軟成員）的軟成員改用錨點的精確
        # (w,h)。V_mib 數的是整個群組的相異形狀數，錨點的尺寸鎖死不能動，所以
        # 只要軟成員形狀跟它不同就一定計違規。
        #
        # **關鍵：在這裡（輸出階段）改，不是在 shapes() 裡改。** 先前試過在
        # shapes() 就鎖死，等於從第 0 輪就拿掉長寬比這個自由度，實測單案
        # area_gap 暴增到 183.9%、cost 1.8485 -> 5.335，慘敗。改在輸出階段做，
        # 優化過程完全不受影響；改形狀造成的小重疊，緊接著的 legalize() 本來
        # 就是設計來吸收這種「接近合法」的輸入的。保留中心點以最小化位移。
        #
        # 面積安全：實測 100 個 MIB 群組全部等面積，且所有混合群組的錨點
        # w*h 都精確等於群組面積，所以採用錨點形狀後面積仍精確命中目標。
        # 仍然逐一驗證，不符合就跳過該群組（嚴格加法式）。
        if os.environ.get("ELECTRO_MIB_ANCHOR", "0") == "1":
            mib_l = mib_id.cpu().tolist()
            a_l = a.cpu().tolist()
            gmax_l = max(mib_l) if mib_l else 0
            for g in range(1, gmax_l + 1):
                grp_mem = [i for i in range(N) if mib_l[i] == g]
                if len(grp_mem) < 2:
                    continue
                rig = [i for i in grp_mem if ispre_l[i] or isfix_l[i]]
                soft_m = [i for i in grp_mem if not (ispre_l[i] or isfix_l[i])]
                if not rig or not soft_m:
                    continue
                tgt_a = a_l[grp_mem[0]]
                if any(abs(a_l[i] - tgt_a) > 1e-6 * max(1.0, tgt_a) for i in grp_mem):
                    continue                      # 面積不一致 -> 形狀本就不可能同
                aw, ah = float(tpl[rig[0]][2]), float(tpl[rig[0]][3])
                if aw <= 0 or ah <= 0:
                    continue
                if abs(aw * ah - tgt_a) > 1e-6 * max(1.0, tgt_a):
                    continue                      # 採用錨點會破壞面積硬約束
                for i in soft_m:                  # 保留中心點，位移最小
                    cx_i = float(x[i]) + 0.5 * float(W[i])
                    cy_i = float(y[i]) + 0.5 * float(H[i])
                    x[i] = cx_i - 0.5 * aw
                    y[i] = cy_i - 0.5 * ah
                    W[i] = aw
                    H[i] = ah
