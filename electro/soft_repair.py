...
def boundary_snap(x, y, w, h, bcode, is_pre, passes=3):
    """Slide each boundary block onto its required bbox edge, searching along the
    wall for a free slot (not just the exact current spot)."""
...
def grouping_repair(x, y, w, h, clust_id, is_pre, passes=4):
    x = np.asarray(x, float); y = np.asarray(y, float)
    w = np.asarray(w, float); h = np.asarray(h, float)
