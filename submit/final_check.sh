...
echo "=== 1) syntax / bytecode compile (all 5 .py) ==="
...
grep -hoE '^(import|from) [a-zA-Z_][a-zA-Z0-9_.]*' "$PKG"/*.py \
  ...
  | grep -vE '^(__future__|os|sys|time|typing|multiprocessing|analytical_place|legalize|soft_repair|electro_parallel|iccad2026_evaluate)$' \
  | sed 's/^/  needs: /'
