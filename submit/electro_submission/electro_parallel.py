...
def run_start_random_with_iters(seed, P, iters):
    """Random-init start at a REDUCED iteration count -- the cheap hedge track.

    Jacobi init is better on average but occasionally lands in a bad basin
    (measured: tid6 4.86 under jacobi-replace vs 2.27 under random init), and
    "replace" mode has no fallback when that happens.  Full "portfolio" mode
    fixes it but pays for a second complete 600-iter place().  This runs the
    random-init track SHORT: enough to reach a different basin the ranking can
    fall back on, without paying full price for a track that usually loses.
    """
    import os
    old = os.environ.get("ELECTRO_INIT", "random")
    ...
    try:
        ...
        positions, _ = place(
            P_custom["n"], P_custom["area"], P_custom["b2b"], P_custom["p2b"],
            P_custom["pins"], P_custom["cons"], P_custom["tp"],
            iters=iters, lr=P_custom["lr"], device=P_custom["device"],
            seed=seed, init_centers=P_custom["init"],
        )
        return post_place_repair(positions, P_custom)
    finally:
        os.environ["ELECTRO_INIT"] = old
...
def compact_variant(start, P, aware=False):
    ...
    ADDITIONAL candidate (ported from the temp branch, commits 0c665b0 / 87ce991).

    Kept verbatim in behaviour, but the repair calls now pass bcode/mib_id/clust_id
    so the compacted layout gets the same constraint-aware repair the rest of this
    module uses -- the older two-arg repair would undo boundary/MIB work the
    surrounding portfolio relies on.

      * aware=False (plain): maximum compaction, constraint-blind.
      * aware=True (S1): clusters compact as RIGID bodies, then the repair chain
        re-snaps boundary blocks onto the new tighter bbox.
    """
    import os
    ...
    cx, cy, cw, ch = compact_and_shape(
        x, y, w, h, P["is_soft"], P["is_pre"], mib_id, ar_cap=ar, floor=floor,
        clust_id=clust_id if aware else None)
    if aware:
        for _ in range(2):
            cx, cy = grouping_repair(cx, cy, cw, ch, clust_id, P["is_pre"],
                                     floor=floor, bcode=bcode, mib_id=mib_id)
            cx, cy = boundary_snap(cx, cy, cw, ch, bcode, P["is_pre"],
                                   floor=floor, clust_id=clust_id, mib_id=mib_id)
