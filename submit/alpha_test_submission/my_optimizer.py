...
# Make this submission folder importable regardless of the grader's current
# working directory, so `floorplan_base` and `ml` always resolve to the files
# shipped alongside this entry point.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Import everything from the baseline -- we only ADD a warm-start step;
# nothing else changes.
from floorplan_base import (
...
class MyOptimizer(_BaselineOptimizer):
    ...
    def __init__(self, verbose: bool = False):
        ...
        # Locate predictor weights (production model shipped in ml/weights/).
        default_weights = Path(__file__).resolve().parent / "ml" / "weights" / "floorplan_v2.pt"
