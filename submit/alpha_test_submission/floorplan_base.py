...
def _physical_core_count() -> int:
    """Number of PHYSICAL CPU cores -- we run one parallel SA chain per physical
    core.  Hyperthreads share a core's execution units, so launching a compute-
    bound chain on every *logical* core oversubscribes and runs SLOWER for this
    workload (measured: 16 chains were slower than 8 on an 8-physical/16-logical
    box).  Parse /proc/cpuinfo topology; fall back to the affinity-limited
    logical count, then os.cpu_count()."""
    try:
        pairs = set()
        phys = core = None
...
class MyOptimizer(FloorplanOptimizer):
    ...
    def __init__(self, verbose: bool = False):
        ...
        # to 48 chains, all running the same per-case wall-clock budget in
